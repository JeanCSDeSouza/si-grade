...
/**
 * The class deals with all the parsing logic to instantiate an student from the
 * PDF text
 * <p>
 * 
 *
...
public class StudentParser {
	 ...
	 * 
	 * @param pdfLines
	 *            The text from the historic PDF
		...
		if (pdfLines.isEmpty()) {
			...
			pdfLines.forEach((line) -> {
				if (line.contains(PdfStudentConstants.STUDENT_PERIOD_CR.getValue()))
					periodsCr.add(Float.parseFloat(line
							.substring(line.length() - PdfStudentConstants.STUDENT_PERIOD_CR.getBegin(),
									line.length() - PdfStudentConstants.STUDENT_PERIOD_CR.getEnd())
							.replace(PdfCharacterRaplacer.PDF_DECIMAL_MARKER.getValue(),
									PdfCharacterRaplacer.PDF_DECIMAL_MARKER_REPLACER.getValue())));
				else {
					if (line.contains(PdfStudentConstants.STUDENT_NAME.getValue()))
						...
						if (line.contains(PdfStudentConstants.STUDENT_REGISTRY.getValue()))
							...
							if (line.contains(PdfStudentConstants.STUDENT_CRA.getValue()))
								student.setCra(
										Float.parseFloat((line.substring(PdfStudentConstants.STUDENT_CRA.getBegin(),
												PdfStudentConstants.STUDENT_CRA.getEnd())).replace(
														PdfCharacterRaplacer.PDF_DECIMAL_MARKER.getValue(),
														PdfCharacterRaplacer.PDF_DECIMAL_MARKER_REPLACER.getValue())));
						}
					}
				}
