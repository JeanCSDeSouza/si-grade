...
public class SvgParser {
	public static List<String> parseSvgLinesColoring(Student student, List<String> svgLines) {
		...
		for (int i = 0; i < lines.size() - 1; i++) {
			if (RegexConstants.isDisciplinePath(lines.get(i))) {
				...
				} else if ((flag.contains("ELETIVA"))) {
					sparring = disciplines.values().stream().filter(discipline -> discipline.getType() == 2)
							.findFirst();
					if (sparring.isPresent()) {
						lines.set(i, paintLine(line, sparring.get()));
						disciplines.remove(sparring.get().getCode());
					}
				} else if (flag.contains("OPTATIV")) {
					sparring = disciplines.values().stream().filter(discipline -> discipline.getType() == 1)
							.findFirst();
					if (sparring.isPresent()) {
						lines.set(i, paintLine(line, sparring.get()));
						disciplines.remove(sparring.get().getCode());
	...
	public static String paintLine(String line, Discipline discipline) {
		...
		if (discipline.getSituation() == 1) {
			...
			if (discipline.getSituation() == 0
					|| ((discipline.getSituation() == -1) && (discipline.getTimesAttended() > 1))) {
				newLine = newLine.replace("ffffff", "FF0000");
			} else if ((discipline.getSituation() == -1) && (discipline.getTimesAttended() == 1)) {
