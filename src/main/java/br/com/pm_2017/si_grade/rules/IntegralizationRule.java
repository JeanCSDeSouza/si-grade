...
/**
 * Checks if the student is in need on delivering an integralization plan. This
 * rule is applied to all the curriculum grades available through reflection.
...
public class IntegralizationRule implements Rule {
	...
	/**
	 * Tests if a student needs to delivery an integralization plan based on the
	 * type of curriculum he is registered using the appropriate validation class.
	 */
	public boolean execute(Student student) {
		...
		return reflected.getReflectedRule(student, RulePackagePrefix.INTEGRALIZATION_RULE_CLASS_NAME.getValue())
				.execute(student);
	}
